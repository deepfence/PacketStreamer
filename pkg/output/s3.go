...
import (
	...
	"github.com/deepfence/PacketStreamer/pkg/buffer"
...
func NewS3BufWriter(config *config.Config) (*buffer.PacketBufWriter, error) {
	writer, err := newS3Writer(config)
	if err != nil {
		return nil, fmt.Errorf("could not create a new S3 writer: %w", err)
	}

	bufWriter := buffer.NewPacketBufWriter(config, writer, int(*config.Output.S3.TotalFileSize))

	pcapBuffer, err := pcap.PcapHeaderBuffer(config)
	if err != nil {
		return nil, err
	}
	if _, err := bufWriter.Write(pcapBuffer); err != nil {
		return nil, fmt.Errorf("could not write the pcap header to S3: %w", err)
	}
